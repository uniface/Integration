...
(function (root, factory) {
    ...
    var Nullformatter = {
        format: function (a_data) {
            // On safari, assigning null to the value property does not change the value
            if (a_data === null) {
                return '';
            }
            return a_data;
...
// %fv:  usyntax.js-27:ascii:1 % %dc:  Tue Sep  8 13:14:41 2015 %
...
// %fv: uhtmlcontrols.js-101.1.3:ascii:1 % %dc: Fri Sep 18 16:39:09 2015 %
...
UNIFACE.widget.element.prototype.setAttributeProp = function(aProp, aValue) {
    ...
    if (element) {
        ...
        try { // ie up to 8 does not accept changes to certain attributes....
            if (aValue === undefined) {
                // Safari does not allow removeAttribute('name') on IMG
                if(!(_uf.is_safari() && aProp == 'name' && element.tagName == 'IMG')){
...
function editableOnKeyUp() {    
    if (this.controlType == "text" || this.controlType == "multiline") {
...
Editable.prototype.mapEvents = function () {
    ...
    if ('oninput' in this.element) {
        this.addListener(this.element, "oninput", l_evs.onedit);
    } else {
        // Workaround for IE8
        this.addListener(this.element, "onkeyup", UNIFACE.bind(editableOnKeyUp, this));
    }
...
UNIFACE.widget.input.prototype.canChangeTypeWhenInDOM = function(fromType, toType) {
    ...
    try {
        ...
        document.documentElement.removeChild(dummyDiv);
        return true;
    } catch (e) {
        document.documentElement.removeChild(dummyDiv);
        return false;
...
UNIFACE.widget.select.prototype.setValue = function (aVal) {
    var doSelect;
    if (!this.element) {
        return;
    }
    var props = this.callBack.getCalculatedProperties();
    if(props.html.multiple == "true"){
        this.setHtmlProp("multiple", true);
    }
    
    aVal = UNIFACE.luv.util.uListToArray(aVal);
    this.setElementValue(aVal[0]);

    var options = this.element.options;

    for (var j=0; j<options.length; j++) {
        doSelect = false; // unselect by default
        // check if item has to be selected 
        for (var i=0; i<aVal.length; i++) {
            doSelect = (options[j].value == aVal[i]);
            if (doSelect){
                break;
            }
        }
        options[j].selected = doSelect;
        // set first selected in case multiple attribute is set to false
        if(!this.element.multiple && doSelect) {
            return;
        }               
    }   
};

UNIFACE.widget.select.prototype.getValue = function() {
    if (!this.element) {
        return this.callBack.getValue();
    }
    var options = this.element.options;
    ...
    for (var i=0; i<options.length; i++) {
        // check if selected
            if(options[i].selected){
                //return first selected in case multiple attribute is set to false
                if(!this.element.multiple) return options[i].value;
                myOptions.push( options[i].value );
        }
    }
    return UNIFACE.luv.util.arrayToUList(myOptions); 
};
